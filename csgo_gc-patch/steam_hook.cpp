...
static void Hk_SteamGameServer_RunCallbacks()
{
    ...
    if (s_serverGC)
    {
        // Revival uses a local fake GC. Local GC delivery must NOT depend on
        // Steam master/GC login state: otherwise a temporary BLoggedOn()==false
        // leaves 9105 sitting forever in the local host-event queue and Source
        // can never produce 9106.
        assert(s_steamGameServer);
        static bool s_printedRevivalOfflineDelivery = false;
        if (!s_printedRevivalOfflineDelivery)
        {
            Platform::Print("REVIVAL_SERVER_GC_OFFLINE_DELIVERY_V1 active\\n");
            s_printedRevivalOfflineDelivery = true;
        }
